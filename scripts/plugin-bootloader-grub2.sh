...
dlib::plugin::bootloader::install() {
    ...
    chroot "${DLIB_MOUNT_ROOT}" "${GRUB_INSTALL[@]}" --target=x86_64-efi --recheck --force --skip-fs-probe --efi-directory=/boot/efi --no-nvram --removable

    # Legacy
    >&2 printf "[*] GRUB: legacy\n"
    # Notes:
    # - QEMU/SeaBIOS requires either `--compress=xz --core-compress=xz` or `--disk-module=native` to boot; otherwise GRUB2 resets itself on any disk read (e.g. during the `search` command)
    ...
    chroot "${DLIB_MOUNT_ROOT}" "${GRUB_MKCONFIG[@]}" -o /boot/grub/grub.cfg
}
