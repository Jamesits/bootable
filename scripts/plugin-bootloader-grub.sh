...
dlib::plugin::bootloader::install() {
    ...
    chroot "${DLIB_MOUNT_ROOT}" grub-install --target=x86_64-efi --recheck --force --skip-fs-probe --efi-directory=/boot/efi --no-nvram --removable

    # Legacy
    >&2 printf "[*] GRUB: legacy\n"
    chroot "${DLIB_MOUNT_ROOT}" grub-install --target=i386-pc --compress=xz --core-compress=xz --recheck --force --skip-fs-probe "${DLIB_DISK_LOOPBACK_DEVICE}"
    ...
    chroot "${DLIB_MOUNT_ROOT}" grub-mkconfig -o /boot/grub/grub.cfg
}
