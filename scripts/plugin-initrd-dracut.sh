...
dlib::plugin::initrd::generate() {
    ...
    if [ "${DLIB_PLUGIN_INITRD_DRACUT_CAVEAT_REGENERATE_ALL}" == "1" ]; then
        chroot "${DLIB_MOUNT_ROOT}" /bin/bash -c "export PATH=/usr/sbin:/sbin:/usr/bin:/bin:$PATH; dracut --regenerate-all --force"
    else
        ...
        chroot "${DLIB_MOUNT_ROOT}" /bin/bash -c "export PATH=/usr/sbin:/sbin:/usr/bin:/bin:$PATH; (rpm -qa | grep kernel | grep -v headers | grep -v tools | sed 's/^kernel\-//g' | xargs -i dracut -v -f /boot/initramfs-{}.img {})"
    fi
}
