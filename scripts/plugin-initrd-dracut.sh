...
bootable::plugin::initrd::generate() {
    ...
    if [ "${BOOTABLE_PLUGIN_INITRD_DRACUT_CAVEAT_REGENERATE_ALL}" == "1" ]; then
        bootable::util:chroot "${BOOTABLE_MOUNT_ROOT}" /bin/bash -c "set -Eeuo pipefail; export PATH=/usr/sbin:/sbin:/usr/bin:/bin:\$PATH; dracut --regenerate-all --force ${DRACUT_ARGS[*]}"
    else
        ...
        bootable::util:chroot "${BOOTABLE_MOUNT_ROOT}" /bin/bash -c "set -Eeuo pipefail; export PATH=/usr/sbin:/sbin:/usr/bin:/bin:\$PATH; (rpm -qa | grep kernel | grep -v headers | grep -v tools | sed 's/^kernel\-//g' | xargs -i dracut -v ${DRACUT_ARGS[*]} -f /boot/initramfs-{}.img {})"
    fi
}
